...
ITEM_LIST=[u'苟利',u'苟利国家生死以，岂因祸福避趋之',u'人在做，天在看',u'大学之道，在明明德',u'量小非君子，无毒不丈夫',u'大江歌罢掉头东，邃密群科济世穷。面壁十年图破壁，难酬蹈海亦英雄。',u'神女应无恙，当惊世界殊。',u'雄关漫道真如铁，而今迈步从头越',u'昔我往矣，杨柳依依。今我来思，雨雪霏霏。',u'硕鼠硕鼠，无食我黍',u'安能摧眉折腰事权贵，使我不得开心颜！',u'我心伤悲，莫知我哀',
           u'我无所谓！',u'图样图森破',u'上台拿衣服',u'月上柳梢头，人约黄昏后。',u'落花人独立，微雨燕双飞。',u'苟日新，日日新。',u'苟富贵，勿相忘。',u'待到秋来九月八，我花开后百花杀。冲天香阵透长安，满城尽带黄金甲。',u'岁寒，然后知松柏之后凋也。',u'穷且益坚，不坠青云之志',u'周公吐哺，天下归心。',u'山不厌高，海不厌深。',u'榆柳荫后檐，桃李罗堂前。'
           ]
...
def text_reply(msg):
    ...
    if friend['NickName'] in [u'飘荡在北方',u'Phantom']:
        if msg['Type'] in ['Text','Note']:
            info=msg['Content'].replace(' ','')
            if not re.findall(r"(傻|sb|SB)", info):
                return '%s  %s\n %s' % (msg.text, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()),random.choice(random.shuffle(ITEM_LIST)))
...
def text_reply(msg):
    info=msg.User
    if info[u'NickName'] in [u'56666',u'hola']:
        if msg.ActualNickName!=u'Van.ziran':
            msg.user.send(u'@%s 回复: %s @@%s' % (
                msg.actualNickName, msg.text,random.choice(random.shuffle(ITEM_LIST))))
